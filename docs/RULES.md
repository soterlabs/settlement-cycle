...
- **Utilized USDS = cum_debt − alm_proxy_usds − psm_usds − sde_asset_value − curve_idle_usds − lending_idle_usds**
  ...
  | `psm_usds` | Idle USDS-equivalent at any PSM (any chain, any `PsmKind`). For PSM3 (`erc4626_shares`) only the **USDS leg** of the basket reduces `utilized` directly; the **USDC leg** is added to `sde_asset_value` (Sky Direct Exposure per Atlas §A.2.3.2.2.3 — also excluded from BR base, with the actual yield routed to Sky); the **sUSDS leg** stays in the BR base and the prime is credited 30 bps × value × n_days as Prime Revenue (Rule 5 — same shape as sUSDS in active allocations, neutralises the SSR-via-share-price + BR-charge composite). See PRD §17.11. Today: Ethereum directed_flow returns $0 for Grove + Spark (mainnet LITE-PSM is non-custodial — defensively configured); L2 PSM3 holdings ~$544M total for Spark as of 2026-05 (≈ 8% USDC SDE, 20% USDS reimbursed, 72% sUSDS BR-charged-with-30bps-credit-back). |
