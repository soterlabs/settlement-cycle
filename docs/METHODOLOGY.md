...
For any venue where the prime holds **sUSDS** (the Sky Savings vault token or a per-chain canonical wrapper), the standard MtM formula overcounts Prime Revenue. The SSR appreciation (`value_eom − value_som`) is generated by Sky's own savings mechanism and returns to Sky via the borrow-rate charge on `utilized`. Crediting it to the prime double-counts.
