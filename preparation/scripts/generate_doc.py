...
def dr_recipe(intro: str) -> str:
    return (f"{intro} Recalculation: OpenMSC rev = sum over the prime’s referral codes of the "
            f"monthly columns in [dr_comparison_2026.xlsx]({DR_XLSX}) (sheet “Soter Data”; the "
            f"referral code is the integer prefix of the ref_code cell, e.g. “0 (sUSDS)” → code 0; "
            f"codes absent from the payouts mapping are excluded). Past payout = sum over the same "
            f"codes of that month’s column in the [DR monthly payouts spreadsheet]({SHEET}) "
            f"(monthly history currently ends 2026-03; machine-readable [CSV export]({SHEET_CSV}) "
            f"— same source used by dr_aggregate.py).")
...
""")
    ...
    def std_prime(prime: str, label: str, warning: str | None,
                  ar_note: str, dr_note_intro: str, dr_extra: str = "") -> None:
        """Spark/Grove/Obex shape; Grove passes extras via dr_extra/chronicle."""
        p.append(f"### {label}\n")
        if warning:
            p.append(warning + "\n")
        p.append("**1. Supply-side revenue**\n")
        p.append(f"""- a. {label} revenue
...
{msc_table(prime, row_of(prime, data, "agent rate"), None)}
    - BA Labs calculated it as well""")
        if prime == "obex":
            p.append(f"""- b. Distribution rewards
    - No active demand-side primitive instances for Obex (no referral codes in the [DR monthly payouts spreadsheet]({SHEET}); confirmed in [MSC#5]({FORUM["MSC#5"]}) through [MSC#8]({FORUM["MSC#8"]}) forum posts).
""")
        else:
            p.append(f"""- b. Distribution rewards
    - OpenMSC calculated it using its [DR repo]({DR_REPO})
        - {dr_recipe(dr_note_intro)}
...
""")
    ...
    # ── Spark ──
    spark_codes = ", ".join(data["codes"]["spark"])
    std_prime(
        "spark", "Spark", None, AR_NOTE_STD,
        f"Note: Spark referral codes were retrieved from [DR monthly payouts spreadsheet]({SHEET}): {spark_codes}.",
    )
    p.append(f"""- f. [Historical Reconciliation] Demand-side primitives
...
""")
    ...
    # ── Grove ──
    grove_codes = ", ".join(data["codes"]["grove"])
    std_prime(
        "grove", "Grove",
        "⚠️ **Important note: numbers provided by OpenMSC have not applied TGE penalty yet**\n",
        AR_NOTE_STD,
        f"Note: Grove referral codes were retrieved from [DR monthly payouts spreadsheet]({SHEET}): {grove_codes}. "
        "Code 2002 “Grove (Maple)” counts as Grove — Maple is Grove’s partner venue (Grove’s March total then matches the MSC#7 forum demand sub-total).",
    )
...
""")
    ...
    # ── Skybase ──
    skybase_codes = ", ".join(data["codes"]["skybase"])
    p.append(f"""### Skybase
...
- b. Distribution rewards
    - OpenMSC calculated it using its [DR repo]({DR_REPO})
        - {dr_recipe(f"Note: Skybase referral codes were retrieved from [DR monthly payouts spreadsheet]({SHEET}): {skybase_codes}.")}

{dr_table(data["dr"]["skybase"], data["payouts"]["skybase"])}
    - BA Labs calculated it as well

- c. Governance Accessibility Rewards (GAR)
    - Total amount: to be filled — applies from May 19, 2025 ([Atlas ref]({ATLAS_GAR})).
- f. [Historical Reconciliation] Demand-side primitives
...
- b. Distribution rewards
    - OpenMSC calculated it using its [DR repo]({DR_REPO})
        - Note: Keel’s referral code per the [DR monthly payouts spreadsheet]({SHEET}) is 4001 (Solana Bridge). The DR results file ([dr_comparison_2026.xlsx]({DR_XLSX}), sheet “Soter Data”) carries NO rows for code 4001, so OpenMSC rev reads 0 while past payouts show 29,062 for March. Code 4001 IS present in the payouts mapping (it would be summed automatically once a results row appears) — what’s missing is a Dune query for the Solana Bridge in the DR repo pipeline. To be investigated. Past payout = code 4001’s row in the [DR monthly payouts spreadsheet]({SHEET}) (monthly history currently ends 2026-03; machine-readable [CSV export]({SHEET_CSV}) — same source used by dr_aggregate.py).
