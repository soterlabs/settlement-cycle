"""Capture Grove June-2026 Dune fixture set.

Same query inventory as the May capture, pinned to June 30 EoM blocks.
SoM = May EoM (one-block-back is fine; the cum_balance series spans
prime-start onward and the runner reads the SoM block directly).

June-30 23:59:59 UTC EoD blocks resolved 2026-07-01 via
``rpc.find_block_at_or_before`` (Dune evms.blocks for eth/base/avax,
RPC binary search for plume/monad). Eth block 25433938 == 23:59:59 UTC
(25433939 is already 2026-07-01T00:00:11).
"""
...
from settle.domain.pin_blocks import month_boundaries  # noqa: E402
from settle.extract.dune import execute_query  # noqa: E402

QUERIES = _REPO / "src" / "settle" / "queries"
OUT = _REPO / "replay" / "grove_2026_06"
OUT.mkdir(parents=True, exist_ok=True)

# June 2026 pin blocks — single source of truth in config/pin_blocks.yaml.
# SoM = May 31 EoM. (str-keyed; extra spark-only chains are ignored here.)
_PINS = month_boundaries(2026, 6)
PIN_BLOCKS_SOM = _PINS["som"]
PIN_BLOCKS_EOM = _PINS["eom"]
...
def main() -> int:
    ...
    print(f"June 2026 fixture capture (Eth EoM = {eth_eom})")
    fx = {
        "_about": f"Grove June 2026 fixture, captured via Dune on 2026-07-01; "
    ...
    for vid, atoken in ATOKENS.items():
        ...
        # block instead of bucketing to end-of-day.
    ...
    # principal-preserving moves from off-chain yield.
    ...
    # eoa_outflow_e36 — paired_principal_cap for E14.
    ...
    with open(_REPO / "replay/grove_2026_05/dune_outputs.json") as f:
        prev = json.load(f)
    ...
    for chain in ("ethereum", "base", "avalanche_c", "plume", "monad"):
        ...
        df = execute_query(
            ...
            params={"chain": chain, "start_date": START_DATE, "end_date": "2026-06-30"},
            pin_block=pb,
        )
