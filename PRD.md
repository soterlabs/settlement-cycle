...
- **Capped SDE** — for `kind: capped`, sd_share is computed daily: `sd_share_d = min(cap_usd, v_d) / v_d`, `sd_revenue = Σ_d daily_rev_d × sd_share_d`. `sd_share_avg = sd_revenue / actual_revenue` is the reported summary figure. JAAA Feb 2026 (legacy SoM-locked): prime $241K, Sky $611K.
