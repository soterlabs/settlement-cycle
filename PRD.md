...
* **Open (1/51 venues):** S23 Anchorage. Position is on-chain visible (escrow + holding wallet known, interest sweeps observed monthly — see §17.12 "Anchorage" entry). YAML is now `skip: true` with the escrow EOA captured as `holder_override` and the loan terms documented inline, so the venue currently contributes a known $0 (rather than the silent ~$0 the old Cat E + const_one + EOA-as-token combination was returning). Build TODO: implement the `TRI_PARTY_LOAN` pricing category (principal-out from cumulative SLL→escrow flow, interest from escrow→SLL flow, optional accrued from APR×Δt). With it enabled, Q1 2026 Spark `prime_agent_revenue` gains ≈ $2.59M and the +$0.4M/mo `monthly_pnl` bias closes.
...
* **Anchorage S23 — $150M tri-party loan, FULLY ON-CHAIN VISIBLE (corrected 2026-05-05).** Earlier (2026-05-01) we wrongly concluded "no on-chain footprint" for this position. Re-verification on 2026-05-05 found the complete flow on-chain via Etherscan:
  ...
  **Pricing-path implication:** we no longer need "a Spark-team-confirmed off-chain feed" to reconstruct this position — every input is on-chain readable:
  * principal-out timeseries: cumulative `Transfer(SLL → 0x49506C3A…6872)` minus `Transfer(0x49506C3A…6872 → SLL)` for USDC.
  * interest realised: same `Transfer(0x49506C3A…6872 → SLL)` flow, classified as Cat C off-pool yield.
  * accrued unrealised interest at any block: `principal × loan_apr × Δt_since_last_sweep` (loan terms from `result_spark_anchorage_usdc` or hardcoded YAML).

  **Build TODO (S3 follow-up):** add a `TRI_PARTY_LOAN` pricing category that consumes the two known addresses + loan APR. With this enabled, Q1 2026 Spark `prime_agent_revenue` increases by ≈ $2.59M (sum of Jan/Feb/Mar interest sweeps), `sky_revenue` charges BR on the $150M principal-out timeseries (≈ $0.5M/month), and the monthly_pnl bias closes. Until the path is built, the previously-stated $0.8M/month under-count of revenue and +$0.4M/month bias on monthly_pnl still stand.
...
- **Anchorage S23 — interest-capture wired up (2026-05-05, partial close on issue #17)** — added the Anchorage Spark escrow EOA to Spark's `external_alm_sources` so monthly USDC sweeps from the escrow flow through Cat A par-stable accounting into `prime_agent_revenue`. To prevent principal-correction events from being mis-classified as yield (the $5M USDC return on 2025-12-19, and the future ≈$150M loan-termination return on 2026-06-16), introduced a new `principal_return_overrides: {chain: {address: [(date, amount)]}}` config block + classifier hook (`PrincipalReturnOverride` domain type + `_cat_a_capital_inflow_timeseries` ``principal_return_overrides=`` parameter). Net effect for Q1 2026: Spark `prime_agent_revenue` gains $891K + $891K + $805K = +$2.59M from the three Q1 interest sweeps. **Sky-revenue side still missing** — BR is not yet charged on the $150M principal-out, so `monthly_pnl` bias narrows but doesn't fully close. The full close still requires the `TRI_PARTY_LOAN` pricing path that issue #17 owns; this PR is a stepping stone, not a full close.
