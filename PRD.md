...
- **Spark SubProxy mid-period USDS flows masked by the fixture loader (follow-up).** The new EoM cross-check fires Apr 2026 (+$525,726) and May 2026 (+$123,680) for the Spark SubProxy. Root cause investigated 2026-06-09: Dune `tokens.transfers` HAS all the events (verified via direct query — 23 USDS Transfer rows match the on-chain delta to the cent), so this is NOT a Dune indexing gap. The culprit is `tests/fixtures/spark_fixture_loader.py:313` which returns `_empty_balance_df()` for any un-routed (token, holder) pair, with a comment stating "Spark Eth subproxy/ALM raw USDS+sUSDS confirmed ~$0 (dust)" — that assertion was written when `config/spark.yaml` listed the urn (`0x691a…`, legitimately $0) as the subproxy. With the corrected SubProxy address (`0x3300…f8c4`, holds $30–37M USDS with monthly flows), the empty-df fallback now masks real movement. The SoM anchor still pegs each month's opening balance to on-chain truth, but mid-period flows are dropped. Time-weighted-average impact on `agent_rate`: ~$300/month max (e.g. Apr: $525K mid-period × ~4 days × SSR+20bps/365 ≈ $300; May: ~$70) — immaterial for headline numbers but conceptually incorrect. **Fix path:** either capture subproxy USDS/sUSDS into the Spark fixture (parallel to `cat_b_cum_balance.json`), or route the subproxy queries through live Dune from the fixture loader.
