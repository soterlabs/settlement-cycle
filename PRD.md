...
- sUSDS in allocation modules (e.g. S32 sUSDS POL) is **not** deducted from `utilized`. Prime Revenue for these venues is **30 bps spread only** (BR − SSR), not the full SSR appreciation. The SSR accrues via the token index and is returned to Sky via the BR charge; crediting it as Prime Revenue would double-count. Implementation: Cat B yield-bearing venues set `actual_revenue_override = value_som × 30bps_daily × n_days` in `VenueRevenueInputs`. See `docs/METHODOLOGY.md §1` and `docs/RULES.md Rule 5`.
