...
11. **Cat A transit misclassification — counterparty logs are load-bearing (review-ack 2026-07-10).** Two confirmed incidents of the same class: a Cat A venue's balance moved in-period while its `inflow_by_counterparty` capture was missing, so the Cat A classifier booked the balance delta as ±yield instead of capital — Grove **E13** (RLUSD transit: +$49,596 May / −$49,596 June 2026, fixed and restated in PR #148) and Grove **E32** (USDC alt-holder: −$3,595 Mar / −$2,254 Apr, below the $50K Jan–Apr materiality bar — documented, not restated). Coverage swept: every Cat A venue in both primes now has its counterparty log captured (Spark was already complete). **Structural guard added** (`_cat_a_capital_inflow_timeseries`): net in-period movement (>$0.01) + empty counterparty log now raises instead of silently misclassifying; venue-scoped escape hatch `SETTLE_ALLOW_UNCLASSIFIED_CAT_A=<venue-ids|1>`. Boundary-neutral transits (in-and-out within the month) are exempt — they cannot affect revenue. Relates to QUESTIONS.md G13 (E13–E18 idle ALM holdings).
