...
**Update 2026-05-13 — resolved on-chain via Option A (`external_alm_sources` + Cat C external-rewards path).** Dune verification (query 7489308) showed Merkl claims TO Grove ALM on **Feb 6 2026** (≈$3.78M aTokens) and **Apr 24 2026** (≈$2.39M aTokens), delivered as `aEthRLUSD` + `aHorRwaRLUSD` (not the underlying RLUSD). This is exactly BA's preferred boundary: capture rewards when they hit the ALM as a stable-ish receipt. New helper `_atoken_external_revenue_usd` (`normalize/positions.py`) queries Dune for transfers from any `external_alm_sources` address into the ALM for the venue's aToken; sum × $1 = USD revenue (par-stable underlying assumed). Threaded through a new `VenueRevenue.external_revenue` field that flows to prime 100 % (NOT subject to SDE-splitting). Grove's `external_alm_sources.ethereum` now lists the Merkl proxy. Re-running Feb / Apr 2026 will credit E1 + E3 with the claim amounts (closes the bulk of the $447K Feb gap for E1; Apr brings the remainder). Pre-Feb-6 / mid-March accrual that was never claimed on-chain remains uncredited by design — matches BA's "boundary at ALM ingress" preference rather than mirroring Grove's spreadsheet's accrued-but-unclaimed accounting.
