    ...
    for v in cfg.get("venues", []):
        ...
        if "nav_oracle" in v:
            ...
            nav_oracle = NavOracle(
                ...
                share_decimals=int(no["share_decimals"]) if no.get("share_decimals") else None,
                underlying_decimals=(
                ...
                fallback_share_decimals=(
                    int(no["fallback_share_decimals"])
                    if no.get("fallback_share_decimals")
                    else None
                ),
