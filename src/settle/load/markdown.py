...
def render_markdown(pnl: MonthlyPnL, *, rules_url: str = DEFAULT_RULES_URL) -> str:
    ...
    p("utilized            = cum_debt − cum_alm_usds − cum_psm_usds − cum_sde_asset_value")
    p("                    − cum_curve_idle_usds − cum_lending_idle_usds")
    p("  note: subproxy USDS/sUSDS are treasury/risk capital — they earn agent_rate")
    p("        but are not deducted from utilized (do not correspond solely to ilk debt)")
    p("  cum_curve_idle_usds   = Σ_venues[curve_idle_usds, par-stable coin only]")
    p("                          (alm_lp_d/total_supply_d) × coin_reserve_d  ($1/unit)")
    p("  cum_lending_idle_usds = Σ_venues[lending_idle_usds=true]")
    p("    (balanceOf(alm,spToken_d) / totalSupply(spToken_d)) × balanceOf(spToken, underlying_d)")
    p("```")
    p("")
