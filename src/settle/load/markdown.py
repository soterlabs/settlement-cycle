...
def render_markdown(pnl: MonthlyPnL, *, rules_url: str = DEFAULT_RULES_URL) -> str:
    ...
    p("                    − cum_psm_usds − cum_curve_idle_usds − cum_lending_idle_usds")
    p("  cum_curve_idle_usds   = Σ_venues[curve_idle_usds]   (alm_lp_d/total_supply_d) × coin_usds_d")
    p("    coin_usds_d: par-stable → face value; sUSDS → convertToAssets(balance, block)")
