...
def render_summary(prov: dict) -> str:
    ...
    lines.append(
        ...
        "contains ``sde_revenue (gross)``** — the per-venue SDE-allocated "
        "share (1.0 for fixed SDE, ``sd_share`` per the per-venue table "
        "for capped SDE) was redirected out of "
        "``prime_agent_revenue (gross)`` and into ``sky_revenue (net)``, "
        "so subtracting it once via ``sky_revenue (net)`` is correct; but "
        "interpreting ``prime_agent_revenue (net)`` as the prime's true "
        "profit overstates Sky's claim by exactly the redirected amount "
        "(which equals ``sde_revenue (gross)``, the prime-level sum of "
        "per-venue ``sd_revenue``). For those primes the prime's true "
        "profit is ``prime_agent_revenue (net) + sde_revenue (gross)``. "
        "Sky's net P&L (not a row) = ``sky_revenue (net)`` − "
        "``agent_rate (gross)`` − ``distribution_rewards (gross)`` "
        "(``pol_agent_rate`` and the sUSDS / Curve / PSM3 spread "
        "reimbursements are NOT subtracted again because they're already "
