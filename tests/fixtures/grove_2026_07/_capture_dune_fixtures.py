"""Capture Grove July-2026 (full month) Dune fixture set.

Same query inventory as the June capture. Captured 2026-08-03 with the
actual July-31 EoM pins (resolved via HyperSync binary search and
cross-checked against the June-30 values, which matched the published
June pins exactly). SoM = June 30 EoM. This is the final monthly
settlement.
"""
...
OUT = _REPO / "tests" / "fixtures" / "grove_2026_07"
OUT.mkdir(parents=True, exist_ok=True)

# July 2026 (FULL MONTH) pin blocks. SoM = June 30 EoM. EoM = July 31
# 23:59:59 UTC EoD blocks per chain. Resolved 2026-08-03 via HyperSync
# binary search (June-30 values re-derived the same way matched the June
# fixture pins on every chain).
PIN_BLOCKS_SOM = {
    "ethereum": 25433938, "base": 48037326, "avalanche_c": 89166730,
    "plume": 78267500, "monad": 84784216,
}
PIN_BLOCKS_EOM = {
...
def main() -> int:
    ...
    print(f"July 2026 (full month) fixture capture (Eth EoM = {eth_eom})")
    fx = {
        "_about": f"Grove July 2026 (full month) fixture, captured via Dune on 2026-08-03; "
    ...
    # E42 Galaxy Warehouse — funded 2026-07-21..27, so July carries its
    # notional and its first (test) distribution. Without this section the
    # loader silently registers no (token, payer, ALM) tuple and July books
    # E42 revenue as $0 instead of the observed $0.25.
    ...
    with open(_REPO / "tests/fixtures/grove_2026_06/dune_outputs.json") as f:
        prev = json.load(f)
    ...
    for chain in ("ethereum", "base", "avalanche_c", "plume", "monad"):
        ...
        df = execute_query(
            ...
            params={"chain": chain, "start_date": START_DATE, "end_date": "2026-07-31"},
            pin_block=pb,
        )
    ...
    print("\nJuly (full month) fixture data captured.")
    return 0
