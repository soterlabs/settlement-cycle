...
def test_usds_only_for_31_days():
    ...
    # NOMINAL (2026-09-01): SSR converted at n=12, + 20bps, × days/365.
    apr = apy_to_apr(Decimal("0.04")) + AGENT_RATE_OVER_SSR
    expected = Decimal("20000000") * apr_daily(apr, 31)
...
def test_susds_only_uses_flat_two_pct():
    ...
    expected = Decimal("5000000") * apr_daily(AGENT_RATE_SUSDS_ONLY)
...
def test_combined_usds_and_susds():
    ...
    expected_usds = Decimal("20000000") * apr_daily(
        apy_to_apr(Decimal("0.04")) + AGENT_RATE_OVER_SSR
    )
    expected_susds = Decimal("5000000") * apr_daily(Decimal("0.002"))
    assert abs(rate - (expected_usds + expected_susds)) < Decimal("1e-9")
...
def test_handles_balance_change_mid_period():
    ...
    f = apr_daily(apy_to_apr(Decimal("0.04")) + AGENT_RATE_OVER_SSR)
    # Feb 1: 21,000,000 (still pre-settlement); Feb 2-5: 21,442,327.
