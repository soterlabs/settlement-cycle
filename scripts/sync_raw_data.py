...
def _live_sources() -> Sources:
    return Sources(
        # debt / balance / ssr / position_balance / block_resolver deliberately
        # left None: compute_monthly_pnl merges the prime's YAML ``sources:``
        # overrides into None fields (per-prime backend pilots, e.g. obex on
        # hypersync, tier-2 position_balance) — pinning a registry default here
        # would silently drop the override on THIS entry point only, settling
        # the same prime on different backends than the production runners.
        # Non-overridable families keep their explicit defaults.
